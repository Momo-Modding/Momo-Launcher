...
function formatPlaytime(ms) {
    playtimeText.innerText = `${(ms / 3600000).toFixed(1)} hrs on record`;
}
...
window.electronAPI.onPlaytimeUpdate((e, time) => {
    getActiveInstance().playtime = time;
    formatPlaytime(time);
});
...
checkUpdatesBtn.addEventListener('click', async () => {
    checkUpdatesBtn.disabled = true; checkUpdatesBtn.innerText = "Checking...";
    ...
    checkUpdatesBtn.innerText = "Check for Updates"; checkUpdatesBtn.disabled = false;
});
...
async function loadModsUI() {
    try {
        ...
        githubDatabase.forEach(mod => {
            ...
            let buttonsHTML = isInstalled ? 
                (needsUpdate ? `<button class="btn-update update-mod-btn" data-id="${mod.id}">Update to v${mod.version}</button>` : '') + `<div class="mod-card-actions"><button class="btn-danger uninstall-mod-btn" data-id="${mod.id}">Uninstall</button></div>` :
                `<div class="mod-card-actions"><button class="btn-install install-mod-btn" data-id="${mod.id}">Install Mod</button></div>`;
            const modCard = document.createElement('div'); modCard.className = 'mod-card';
        ...
        Object.keys(inst.installedMods).forEach(id => {
            if (inst.installedMods[id] === "local") {
                const localCard = document.createElement('div'); localCard.className = 'mod-card';
                localCard.innerHTML = `<h3>${id} <small>Local File</small></h3><div class="author">By You</div><div class="mod-card-actions"><button class="btn-danger uninstall-mod-btn" data-id="${id}">Uninstall</button></div>`;
